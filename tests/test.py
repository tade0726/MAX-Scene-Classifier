...
def test_swagger():
    ...
    assert json.get("info") and json.get("info").get("title") == "图片场景分类api"
...
def test_metadata():
    ...
    metadata = r.json()
    assert metadata["id"] == "图片场景分类api"
    assert metadata["name"] == "图片场景分类api"
    assert (
        metadata["description"] == "Pytorch ResNet18 model trained on Places365 dataset"
    )
...
def test_labels():
    ...
    assert labels["labels"][0]["name"]["category"] == "airfield"
    assert labels["labels"][-1]["id"] == "364"
    assert labels["labels"][-1]["name"]["category"] == "zen_garden"
...
def _check_predict(r):
    ...
    assert response["predictions"][0]["label"]["category"] == "bakery/shop"
